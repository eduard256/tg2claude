#!/usr/bin/env python3
"""
Скрипт для включения файлов обратно в загрузку (установка priority=4)
Использование: python qbt-download-files.py <hash> <ID файлов>

Примеры:
  python qbt-download-files.py <hash> 0,1,2       - включить файлы 0, 1, 2
  python qbt-download-files.py <hash> 5-10        - включить файлы с 5 по 10
  python qbt-download-files.py <hash> 0,3,5-8,12  - комбинированный вариант
"""
...
def download_files(hash_id, ids_string):
    """Включить файлы в загрузку"""
    ...
    # Устанавливаем priority=1 (нормальный) для файлов
    success = set_file_priority(session, base_url, hash_id, file_ids, priority=1)

    if success:
        print(f"✅ Файлы включены в загрузку")
        print(f"\n📝 Включённые файлы:")
        for fid in file_ids:
            file_info = files[fid]
            name = file_info['name']
            ...
            print(f"   [{fid}] ✅ {name} ({size}) [{progress:.1f}%]")

        # Считаем сколько будет скачано
        total_download = sum(files[fid]['size'] for fid in file_ids)
        print(f"\n📥 Будет скачано: {format_size(total_download)}")
        return True
    else:
...
def main():
    if len(sys.argv) != 3:
        print("Использование: python qbt-download-files.py <hash> <ID файлов>")
        print("\nПримеры:")
        print("  python qbt-download-files.py <hash> 0,1,2       - включить файлы 0, 1, 2")
        print("  python qbt-download-files.py <hash> 5-10        - включить файлы с 5 по 10")
        print("  python qbt-download-files.py <hash> 0,3,5-8,12  - комбинированный вариант")
        sys.exit(1)

    hash_id = sys.argv[1].lower()
    ids_string = sys.argv[2]

    success = download_files(hash_id, ids_string)
